...
import { a as seedTrips, i as seedMaintenance, n as seedDrivers, o as seedVehicles, r as seedExpenses, s as useStore } from "./_ssr/store-9LEy85iz.mjs";
...
import { C as Moon, D as LayoutDashboard, E as LogOut, L as ChartColumn, M as Database, _ as Route, d as Sun, i as Users, n as X, o as Truck, r as Wrench, w as Menu, y as Receipt } from "./_libs/lucide-react.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/_app-CjHgx8nS.js
...
function AppLayout() {
	...
	(0, import_react.useEffect)(() => {
		if (user) Promise.all([
			...
			const mappedVehicles = vArr.length > 0 ? vArr.map((v) => ({
				...v,
				id: v._id || v.id,
			...
			const mappedDrivers = dArr.length > 0 ? dArr.map((d) => ({
				...d,
				id: d._id || d.id,
			...
			const mappedTrips = tArr.length > 0 ? tArr.map((t) => ({
				...t,
				id: t._id || t.id,
			...
			const mappedMaintenance = mArr.length > 0 ? mArr.map((m) => ({
				...m,
				id: m._id || m.id,
			...
			const mappedExpenses = eArr.length > 0 || fArr.length > 0 ? [...eArr.map((e) => ({
				...e,
				id: e._id || e.id,
