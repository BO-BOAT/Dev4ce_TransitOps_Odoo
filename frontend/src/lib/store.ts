...
export type Role = "admin" | "dispatcher" | "manager" | "viewer";
...
export interface Vehicle {
  ...
  regNumber: string;
  model: string;
  type: "Truck" | "Van" | "Tanker" | "Trailer" | "Pickup";
  maxLoadKg: number;
  odometer: number;
...
export interface MaintenanceRecord {
  ...
  cost: number;
  startDate: string;
  endDate?: string; // undefined = active
}

export type ExpenseKind = "Fuel" | "Toll" | "Parking" | "Other";
...
interface State {
  ...
  createTrip: (t: Omit<Trip, "id" | "status" | "createdAt">) => { ok: boolean; error?: string };
  dispatchTrip: (id: string) => { ok: boolean; error?: string };
  completeTrip: (id: string, actualKm: number, revenue: number) => void;
  cancelTrip: (id: string) => void;

  openMaintenance: (m: Omit<MaintenanceRecord, "id" | "endDate">) => void;
...
export const seedVehicles: Vehicle[] = [
  ...
  { id: "v4", regNumber: "TN-22-GH-3456", model: "Eicher Pro 6055", type: "Trailer", maxLoadKg: 35000, odometer: 45200, acquisitionCost: 5100000, status: "Available", region: "South" },
  { id: "v5", regNumber: "GJ-05-JK-7890", model: "Force Traveller", type: "Van", maxLoadKg: 1800, odometer: 28900, acquisitionCost: 1450000, status: "Available", region: "West" },
...
export const seedDrivers: Driver[] = [
  ...
  { id: "d4", name: "Vikram Singh", licenseNumber: "DL-2298471", licenseCategory: "E", licenseExpiry: "2025-08-20", contact: "+91 89123 99801", safetyScore: 74, status: "Off Duty" },
...
export const seedExpenses: Expense[] = [
  ...
  { id: "e4", vehicleId: "v4", kind: "Fuel", amount: 21000, liters: 240, date: today() },
];
  ...
  persist(
    (set, get) => ({
      ...
      addVehicle: (v) => {
        ...
        if (exists) return { ok: false, error: "Registration number must be unique" };
        set({ vehicles: [...get().vehicles, { ...v, id: uid() }] });
        return { ok: true };
      },
      updateVehicle: (id, v) =>
        set({ vehicles: get().vehicles.map((x) => (x.id === id ? { ...x, ...v } : x)) }),
      deleteVehicle: (id) => set({ vehicles: get().vehicles.filter((x) => x.id !== id) }),

      addDriver: (d) => set({ drivers: [...get().drivers, { ...d, id: uid() }] }),
      updateDriver: (id, d) =>
        set({ drivers: get().drivers.map((x) => (x.id === id ? { ...x, ...d } : x)) }),
      ...
      createTrip: (t) => {
        ...
        if (veh.status === "Retired" || veh.status === "In Shop")
          return { ok: false, error: "Vehicle is not dispatchable" };
        ...
        if (t.cargoKg > veh.maxLoadKg)
          return { ok: false, error: `Cargo exceeds max load (${veh.maxLoadKg} kg)` };
        set({
          trips: [
            ...get().trips,
      ...
      dispatchTrip: (id) => {
        ...
        if (!trip || trip.status !== "Draft") return { ok: false, error: "Trip not in draft" };
        const veh = get().vehicles.find((v) => v.id === trip.vehicleId)!;
        const drv = get().drivers.find((d) => d.id === trip.driverId)!;
        if (veh.status !== "Available") return { ok: false, error: "Vehicle unavailable" };
        if (drv.status !== "Available") return { ok: false, error: "Driver unavailable" };
        if (new Date(drv.licenseExpiry) < new Date())
          return { ok: false, error: "Driver license expired" };
        set({
          trips: get().trips.map((t) => (t.id === id ? { ...t, status: "Dispatched" } : t)),
          vehicles: get().vehicles.map((v) => (v.id === veh.id ? { ...v, status: "On Trip" } : v)),
          drivers: get().drivers.map((d) => (d.id === drv.id ? { ...d, status: "On Trip" } : d)),
        });
        return { ok: true };
      },
      completeTrip: (id, actualKm, revenue) => {
        ...
        set({
          trips: get().trips.map((t) =>
            t.id === id
              ? { ...t, status: "Completed", actualKm, revenue, completedAt: new Date().toISOString() }
              : t,
          ),
          vehicles: get().vehicles.map((v) =>
            v.id === trip.vehicleId ? { ...v, status: "Available", odometer: v.odometer + actualKm } : v,
          ),
      ...
      cancelTrip: (id) => {
        ...
        set({
          trips: get().trips.map((t) => (t.id === id ? { ...t, status: "Cancelled" } : t)),
      ...
      openMaintenance: (m) => {
        set({
          maintenance: [...get().maintenance, { ...m, id: uid() }],
          vehicles: get().vehicles.map((v) =>
      ...
      closeMaintenance: (id) => {
        ...
        set({
          maintenance: get().maintenance.map((m) =>
            m.id === id ? { ...m, endDate: new Date().toISOString() } : m,
          ),
          vehicles: get().vehicles.map((v) =>
      ...
      addExpense: (e) => set({ expenses: [...get().expenses, { ...e, id: uid() }] }),
    }),
    { name: "fleet-store-v1" },
  ),
);

export function exportCSV(filename: string, rows: readonly Record<string, unknown>[] | readonly object[]) {
  const data = rows as Record<string, unknown>[];
  return _exportCSV(filename, data);
}
function _exportCSV(filename: string, rows: Record<string, unknown>[]) {
